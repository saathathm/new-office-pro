...
<div class="">
    <table class="table table-hover rbborder">
        ...
        if ($count_prayer_time > 0) {
            $p_seconds = $value_prayer_time['extra_time'];
        }
        ...
        ?>
            <tr>
                ...
                <td><?php if ($value['logout_time'] != date("H:i:s", strtotime("12:00 AM"))) {
                        echo '<a href="reduce_time.php?logout_time=' . $value['logout_time'] . '&&login_time=' . $value['login_time'] . '&&date=' . $_REQUEST['date3'] . '&&id=' . $value['id'] . '"  rel="facebox">';
                        echo date("h:i a", strtotime($value['logout_time']));
                        echo '</a>';
                    } else {
                        echo "--:--";
                    } ?></td>
        ...
        <?php }
        $seconds = @array_sum($_SESSION['arrayHours']) + $p_seconds;
